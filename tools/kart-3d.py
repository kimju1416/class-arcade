...
for cid in ids:
    if os.path.exists(os.path.join(OUT, f'{cid}.glb')) and cid in man: print(cid, '이미 있음'); continue
    rc = prep(cid)
...
with open(os.path.join(HERE, '..', 'public', 'kart', 'char3d.js'), 'w', encoding='utf-8', newline='\n') as f:
    ...
    f.write('export const CHAR3D = ' + json.dumps({k: {'f': v['front'], 'b': v['back']} for k, v in man.items()}) + ';\n')
