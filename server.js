...
const MIME = {
  ...
  '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.mp3': 'audio/mpeg', '.wav': 'audio/wav',
...
server.on('upgrade', (req, socket, head) => {
  ...
  const target = pathname === '/nexus/ws' ? nexusWss : pathname === '/fps/ws' ? fpsWss : pathname === '/kart/ws' ? kartWss : pathname === '/mallang-7xq4/ws' ? puyoWss : wss;
...
// 심야엔 재워서 무료 시간(월 750h, 같은 계정의 다른 서비스와 공유)을 아낀다.
// RENDER_EXTERNAL_URL은 Render가 자동 주입 — 로컬 실행에는 없어서 아무 일도 안 한다.
// 시간대를 바꾸려면 Render 환경변수 KEEPALIVE_HOURS 로 (예: '6-24', 24시간이면 '0-24').
const SELF_URL = process.env.RENDER_EXTERNAL_URL;
if (SELF_URL) {
  const https = require('https');
  const m0 = String(process.env.KEEPALIVE_HOURS || '7-22').match(/^(\d{1,2})-(\d{1,2})$/);
  const [h0, h1] = m0 ? [+m0[1], +m0[2]] : [7, 22];
  setInterval(() => {
    const kstH = new Date(Date.now() + 9 * 3600 * 1000).getUTCHours();
    if (kstH < h0 || kstH >= h1) return;
