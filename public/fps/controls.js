export function createControls({look,shoot,weaponChanged,pauseChanged}){
 ...
 const c={mobile,keys,paused:true,enabled:false,weapon:'rifle',fire:false,aim:false,trigger:0,reloadTrigger:0,f:0,s:0,board:false};let reloadUntil=0,jumpUntil=0;
 ...
 document.addEventListener('keydown',e=>{if(!c.enabled)return;if(['Tab','Space','KeyW','KeyA','KeyS','KeyD'].includes(e.code))e.preventDefault();if(e.code==='KeyR'&&!keys.KeyR&&!c.paused)c.reloadTrigger++;keys[e.code]=true;if(e.code==='Digit1')c.select('rifle');if(e.code==='Digit2')c.select('sniper')});document.addEventListener('keyup',e=>keys[e.code]=false);document.addEventListener('contextmenu',e=>{if(c.enabled)e.preventDefault()});window.addEventListener('blur',()=>{if(c.enabled)c.pause()});document.addEventListener('visibilitychange',()=>{if(document.hidden&&c.enabled)c.pause()});
 ...
 hold('touch-fire',pressFire,()=>c.fire=false);hold('touch-aim',()=>{c.aim=!c.aim;$('touch-aim').classList.toggle('on',c.aim)});hold('touch-reload',()=>{c.reloadTrigger++;reloadUntil=performance.now()+180});hold('touch-jump',()=>jumpUntil=performance.now()+180);hold('touch-sprint',()=>keys.ShiftLeft=true,()=>keys.ShiftLeft=false);
 return c;
}
