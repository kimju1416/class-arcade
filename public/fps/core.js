export const BOXES=[[-24,3,0,2,6,52,'wall'],[24,3,0,2,6,52,'wall'],[0,3,-26,50,6,2,'wall'],[0,3,26,50,6,2,'wall'],[-10,1.6,-9,6,3.2,12,'blue'],[10,1.6,9,6,3.2,12,'red'],[9,1.6,-13,11,3.2,5,'olive'],[-9,1.6,13,11,3.2,5,'olive'],[0,1.2,0,4,2.4,5,'crate'],[-18,1,3,3,2,3,'crate'],[18,1,-3,3,2,3,'crate'],[2,1,-19,3,2,3,'crate'],[-2,1,19,3,2,3,'crate']];
export const SPAWNS=[[-19,21],[19,-21],[19,21],[-19,-21],[0,22],[0,-22],[-20,0],[20,0]];
export const clamp=(n,a,b)=>Math.max(a,Math.min(b,n));
export function blocked(x,z){return Math.abs(x)>22.5||Math.abs(z)>24.5||BOXES.some(b=>Math.abs(x-b[0])<b[3]/2+.35&&Math.abs(z-b[2])<b[5]/2+.35)}
export function rayBox(o,d,min,max){let lo=0,hi=100;for(let i=0;i<3;i++){if(Math.abs(d[i])<1e-8){if(o[i]<min[i]||o[i]>max[i])return Infinity;}else{let a=(min[i]-o[i])/d[i],b=(max[i]-o[i])/d[i];lo=Math.max(lo,Math.min(a,b));hi=Math.min(hi,Math.max(a,b));if(hi<lo)return Infinity}}return lo}
export function wallDistance(o,d){return Math.min(...BOXES.map(b=>rayBox(o,d,[b[0]-b[3]/2,b[1]-b[4]/2,b[2]-b[5]/2],[b[0]+b[3]/2,b[1]+b[4]/2,b[2]+b[5]/2])))}
export const WEAPONS={rifle:{name:'AR-4',mag:30,interval:.105,reload:1.8,damage:28,head:70},sniper:{name:'SR-7',mag:5,interval:1.25,reload:2.6,damage:100,head:150}};
export class Arena {
 ...
 step(dt){this.events=[];if(this.over){this.restart-=dt;if(this.restart<=0){this.over=false;this.time=300;this.scores=[0,0];for(let p of Object.values(this.players)){p.kills=0;p.deaths=0;this.spawn(p)}}return}this.time-=dt;let all=Object.values(this.players);
  for(let p of all){p.cool=Math.max(0,p.cool-dt);p.shield=Math.max(0,p.shield-dt);if(p.hp<=0){p.respawn-=dt;if(p.respawn<=0)this.spawn(p);continue}
   ...
   if(p.bot){let target=all.filter(q=>q.id!==p.id&&q.hp>0&&(this.mode==='ffa'||q.team!==p.team)).sort((a,b)=>Math.hypot(a.x-p.x,a.z-p.z)-Math.hypot(b.x-p.x,b.z-p.z))[0];if(target){let yaw=Math.atan2(p.x-target.x,p.z-target.z),distance=Math.hypot(target.x-p.x,target.z-p.z),phase=Math.sin(this.time*.6+Number(p.id.slice(3))*2),delta=Math.atan2(Math.sin(yaw-p.yaw),Math.cos(yaw-p.yaw));yaw=p.yaw+clamp(delta,-dt*3,dt*3);p.input={yaw,pitch:0,f:distance>12?1:0,s:phase*.6,fire:distance<32&&Math.abs(delta)<.15&&p.shield===0,reload:p.ammo===0,weapon:'rifle'};if(blocked(p.x-Math.sin(yaw)*.8,p.z-Math.cos(yaw)*.8)){p.input.f=.25;p.input.s=phase>0?1:-1}}}
