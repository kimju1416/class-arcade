...
- 게임 31종 (한 방 최대 40명) · 탈락·득점 연출, 3단 포디움, 학생 이모지 반응, 랜덤 게임 룰렛
  - **새 운동장 게임팩**: 🌼 무궁화 꽃이 피었습니다(75초·초록에 이동/빨강에 정지, 한 번 걸리면 즉시 탈락·관전) · 🎨 색 땅따먹기(70초·이동하며 칠하기, 7초마다 넓게 칠하기, 최종 소유 칸 수) · 🎣 보물 낚시(60초·입질에 챔질→초록 게이지 적중, 연속 성공 보너스)
  ...
  - **기억력**: 🎵 사이먼 가라사대(패턴 따라하기) · 🐵 침팬지 테스트(숫자 위치 기억) · ⚡ 순간 포착(플래시 퀴즈) · 🃏 같은 그림 찾기(짝 맞추기)
- 로비의 게임 목록은 분류 탭(⚔️ 서바이벌 · 🏆 점수 경쟁 · 🧠 퀴즈·두뇌 · 🧩 기억력 · 🎲 복불복)으로 골라볼 수 있다
...
## NEXUS ISLES · 3D 하늘섬 어드벤처

[넥서스 아일즈 플레이](https://game.kimju.kr/nexus/) — 크리스털 12개 수집과 포털 엔딩,
9개 관문의 파쿠르, 4종 재료로 정원 건설. PC와 모바일 터치 조작을 지원합니다.
친구 초대 링크로 최대 16명이 서로의 탐험 위치를 볼 수 있습니다.
개인 기록과 건설물은 각 기기에 저장되며, 건설물은 다른 참가자와 동기화되지 않습니다.
배경음악 3곡(탐험·파쿠르·정원)과 효과음은 Mixkit 무료 음원(`public/nexus/audio/CREDITS.txt`). 생성 이미지 10종을 적용했습니다. 자세한 내용은 `public/nexus/README.md`, 전용 검사는 `npm run test:nexus`.
...
| 🎵 사이먼 가라사대 | 4색 패드 | TV가 연주하는 4색 멜로디를 외워 같은 순서로 입력. 한 음씩 길어짐. 틀리면 탈락, 전원 틀리면 전원 부활 |
| 🐵 침팬지 테스트 | 화면 탭 | TV에 숫자 위치가 잠깐 보였다 사라짐 — 1부터 순서대로 탭. 라운드마다 숫자 증가(4→11개). 점수제 |
| ⚡ 순간 포착 | 4색 버튼 | TV에 그림들이 2초 떴다 사라짐 — "몇 개였을까?", "없었던 것은?" 4지선다. 빠를수록 보너스 |
| 🃏 같은 그림 찾기 | 화면 탭 | 뒤집힌 카드 20장(전원 같은 배치)에서 짝 찾기. 적게 뒤집고 빨리 완주할수록 고득점 |
...
party-games.js     새 게임 3종의 서버 판정 (무궁화·땅따먹기·낚시)
public/party-games.js 새 게임의 Canvas 화면·안내·조작 상태
test/party-games.test.js 새 게임 규칙·중간 입장·재접속 검증
package.json       의존성 (ws 하나뿐)
PRD.md             기획서
```

새 게임도 기존 개인전·팀 대항전·오늘의 챔피언을 사용한다. 게임 중 입장한 학생은
다음 판부터 참가하며, 같은 기기에서 새로고침하면 기존 토큰으로 복귀한다.
새 게임은 선택 바에 시작 전 규칙이 표시된다. 낚시는 버튼만, 무궁화와 땅따먹기는
모바일 조이스틱 또는 PC 방향키/WASD로 조작한다. 낚시와 넓게 칠하기 버튼은
Tab으로 선택 후 Enter/Space로도 누를 수 있다.
